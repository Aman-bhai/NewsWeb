          ...
          <div className="max-h-80 overflow-y-auto">
            {news.comments && news.comments.length > 0 ? (
              news.comments.map((comment, index) => (
                <div
                  key={index}
                  className="flex items-start mb-4 p-3 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-sm"
                >
                  <Image
                    src={`https://avatar.iran.liara.run/username?username=${encodeURIComponent(
                      comment.userEmail
                    )}`}
                    alt={`Avatar of ${comment.userEmail}`}
                    width={50}
                    height={50}
